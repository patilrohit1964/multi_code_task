...
const CardShirt = () => {
    ...
    const ProductCard = ({ delay = 0 }) => (
        <Card
            className={`bg-white rounded-2xl overflow-hidden transform transition-all duration-500 ease-out ${showMore ? 'translate-y-0 opacity-100' : ''
                } pt-0`}
            style={{
    ...
    const ProductSection = ({ title, showViewAll = false }) => (
        <section className="pt-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 max-w-7xl mx-auto gap-6 px-4">
                {[1, 2, 3, 4].map((_, idx) => (
                    <Card key={idx} className="bg-white rounded-2xl overflow-hidden pt-0">
                        <CardHeader className="overflow-hidden p-0">
                            <img
                                src={title === "TOP SELLING" ? '/shirt2.jpeg' : '/shirt1.webp'}
                                alt="T-shirt"
                                className="h-64 w-full object-cover transition-transform duration-300"
                            />
                        </CardHeader>
            ...
            {showViewAll && !showMore && (
                <div className="flex items-center justify-center py-16">
                    <button
                        className="bg-white text-black rounded-full px-8 py-3 font-medium hover:bg-gray-100 transition-all duration-300 cursor-pointer border border-gray-300 hover:border-gray-400 transform"
            ...
            {/* Additional 4 cards with animation */}
            {showViewAll && (
                <div
